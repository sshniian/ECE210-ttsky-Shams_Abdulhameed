...
## Project Overview
Abstract
This project implements a hardware-efficient neuromorphic decision module for relay selection in cooperative wireless communication systems. Based on prior Monte Carlo simulations evaluating Bit Error Rate (BER) performance under different power allocation values (α), the optimal relay selection behavior is translated into a synthesizable digital architecture. Rather than computing BER or Signal-to-Noise Ratio (SNR) in real time, the hardware operates on an 8-bit power allocation parameter α and embeds the derived decision boundary into three predefined α-dependent current functions implemented within sequential Leaky Integrate-and-Fire (LIF) neurons. Relay selection is determined solely through the dynamic competition of neuron membrane voltages under leakage and threshold constraints, with a Winner-Take-All (WTA) mechanism selecting the first neuron to reach activation. Consequently, the implemented circuit performs deterministic, fixed-point policy execution that approximates the relay preference regions observed in simulation, replacing numerical optimization with structured neuromorphic dynamics.
